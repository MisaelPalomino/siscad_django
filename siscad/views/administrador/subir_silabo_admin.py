...
def subir_silabo_admin(request):
    ...
    if profesor_id:
        ...
        grupos_teoria = GrupoTeoria.objects.filter(
            profesor=profesor_seleccionado
        ).select_related("curso")

        # Obtener información de silabos existentes para los grupos del profesor
    ...
    if request.method == "POST":
        ...
        grupo_teoria = get_object_or_404(GrupoTeoria, id=grupo_id_post)
        ...
        if archivo_excel:
            try:
                ...
                temas_creados = procesar_excel_temas_admin(
        ...
        return redirect(
            f"{request.path}?profesor_id={profesor_id}&grupo_id={grupo_id_post}"
        )
    ...
    return render(request, "siscad/admin/subir_silabo_admin.html", context)


def procesar_excel_temas_admin(request, archivo_excel, grupo_teoria):
    """
    Procesa el archivo Excel de temas y distribuye las fechas equitativamente - VERSIÓN ADMIN
    """
    try:
        ...
        fechas_clase = obtener_fechas_teoria_reales_admin(grupo_teoria)
...
def obtener_fechas_teoria_reales_admin(grupo_teoria):
    """
    Obtiene las fechas reales de clase de TEORÍA basadas en las asistencias del profesor - VERSIÓN ADMIN
    """
    try:
        ...
        if asistencias.exists():
            ...
            return obtener_fechas_de_horario_teoria_admin(grupo_teoria)
...
def obtener_fechas_de_horario_teoria_admin(grupo_teoria):
    """
    Obtiene fechas basadas en el horario programado de TEORÍA del profesor - VERSIÓN ADMIN
    """
    try:
        ...
        fecha_inicio, fecha_fin = obtener_rango_semestre_actual_admin()
...
def obtener_rango_semestre_actual_admin():
    """
    Intenta obtener el rango de fechas del semestre actual - VERSIÓN ADMIN
    """
    try:
...
def descargar_plantilla_silabo_excel_admin(request, grupo_id):
    """
    Descarga una plantilla Excel para subir temas del silabo - VERSIÓN ADMIN
    """
    try:
        ...
        fechas_reales = obtener_fechas_teoria_reales_admin(grupo_teoria)
        ...
        with pd.ExcelWriter(output, engine="openpyxl") as writer:
            ...
            info_curso = [
                ...
                f"Profesor: {grupo_teoria.profesor.nombre}",
        ...
        return redirect("subir_silabo_admin")
