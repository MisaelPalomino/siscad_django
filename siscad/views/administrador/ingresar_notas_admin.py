...
def ingresar_notas_admin(request):
    ...
    if grupo_id:
        ...
        # PREPARAR LOS DATOS DE NOTAS CUANDO SE SELECCIONA UN GRUPO VÍA GET
    ...
    if request.method == "POST":
        ...
        if grupo_id_post:
            grupo_seleccionado = get_object_or_404(GrupoTeoria, id=grupo_id_post)
            alumnos_grupo = MatriculaCurso.objects.filter(
                curso=grupo_seleccionado.curso, turno=grupo_seleccionado.turno
            ).select_related("alumno")
            ...
            if "guardar_manual" in request.POST:
                ...
                return redirect(
                    f"{request.path}?profesor_id={grupo_seleccionado.profesor.id}&grupo_id={grupo_seleccionado.id}"
                )
    ...
    return render(request, "siscad/admin/ingresar_notas_admin.html", context)


def descargar_plantilla_excel_admin(request, grupo_id):
    """
    Vista para descargar una plantilla Excel para cargar notas - VERSIÓN ADMIN
    """
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        config_data = {
            "Configuración del Curso": [
                ...
                f"Profesor: {grupo_teoria.profesor.nombre if grupo_teoria.profesor else 'No asignado'}",
                "",
                "Pesos de Evaluación:",
