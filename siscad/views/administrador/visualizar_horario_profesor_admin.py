...
def visualizar_horario_profesor_admin(request):
    ...
    if profesor_id:
        ...
        reservas_semana = Reserva.objects.filter(
            profesor=profesor_seleccionado, fecha__range=(inicio_semana, fin_semana)
        ).select_related("aula")
        ...
        total_reservas_activas = Reserva.objects.filter(
            profesor=profesor_seleccionado, fecha__gte=hoy
        ).count()
        reservas_disponibles = max(
            0, profesor_seleccionado.cantidad_reservas - total_reservas_activas
        )

        estadisticas = {
            "total_horarios": horarios.count(),
            "total_teoria": horarios.filter(
                grupo_teoria__profesor=profesor_seleccionado
            ).count(),
            "total_practica": horarios.filter(
                grupo_practica__profesor=profesor_seleccionado
            ).count(),
            "total_laboratorio": horarios.filter(
                grupo_laboratorio__profesor=profesor_seleccionado
            ).count(),
            "cursos_unicos": len(
                set(
    ...
    return render(
        request, "siscad/admin/visualizar_horario_profesor_admin.html", context
    )
